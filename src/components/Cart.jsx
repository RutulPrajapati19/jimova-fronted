...
const Cart = () => {
  ...
  const handleCheckout = async (customerDetails) => {
    ...
    try {
      ...
      try {
        await axios.post(`${baseUrl}/api/orders/place`, orderPayload, {
          headers: { "Authorization": `Bearer ${token}` }
        });
      } catch (err) {
        console.error("Database Order POST Failed:", err);
        toast.error("Backend refused the order data. Check your Spring Boot console.");
        return; 
      }

      // 2. REPLENISH STOCK FOR UNLIMITED TESTING
      let isStockUpdated = true;
      for (const item of cartItems) {
        const { imageUrl, imageName, imageData, imageType, quantity, ...rest } = item;
        
        // ✦ THE FIX: Instantly restore stock to 100 so it NEVER says "OUT" on your Home page ✦
        const updatedProductData = { ...rest, stockQuantity: 100 };
      ...
      if (isStockUpdated) {
        ...
        toast.warning("Order saved, but stock update had issues.");
      }
